...
Why are `Latitude` and `Longitude` so big? Why does `Gr_Liv_Area` barely show up?
...
workflow1_fit <- ames_workflow_1 %>% fit(data = ames_train)
```


```{r}
workflow1_fit$fit$fit %>% 
  tidy() %>%
  filter(term != "(Intercept)") %>% 
  ggplot(aes(x = estimate, y = fct_reorder(term, estimate))) + geom_col()
```

Which coefficients are the most positive? The most negative? Why might this make sense?
...
```{r}
neighborhood_intercepts_recipe <- 
  recipe(Sale_Price ~ ., data = ames_train) %>% 
...
ames_workflow_2 <- 
  workflow() %>% 
  add_recipe(neighborhood_intercepts_recipe) %>% 
  add_model(linear_reg())

workflow2_fit <- ames_workflow_2 %>% fit(data = ames_train)
workflow2_fit$fit$fit %>% 
  tidy() %>%
  filter(term != "(Intercept)") %>% 
  ggplot(aes(x = estimate, y = fct_reorder(term, estimate), fill = str_starts(term, "Centr"))) + 
    geom_col() +
