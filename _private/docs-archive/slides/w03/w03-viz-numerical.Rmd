...
.pull-left[
```{r barplot-state-y-infreq, fig.asp=1, out.width="100%"}
ggplot(loans, aes(y = fct_infreq(state))) +
  geom_bar()
```
]
...
.pull-right[
```{r barplot-state-y-other, fig.asp=1, out.width="100%"}
# bonus!
ggplot(loans, aes(y = state %>% fct_infreq() %>% fct_lump_n(15) %>% fct_rev())) +
  geom_bar()
```
]
...
```{r manual-counts}
ggplot(loan_proportions, aes(y = homeownership, fill = grade,
                             x = prop)) + #<<
  geom_col() #<<
```
]
